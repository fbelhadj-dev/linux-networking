...
sudo bridge vlan d dev br-ns vid 1 self
sudo bridge vlan a dev br-ns vid 10 pvid self
sudo bridge vlan a dev br-ns vid 20 pvid self

sudo ip link a link br-ns name br-ns.10 type vlan id 10
sudo ip link a link br-ns name br-ns.20 type vlan id 20

sudo ip addr a 10.0.10.254/24 dev br-ns.10
sudo ip addr a 10.0.20.254/24 dev br-ns.20

sudo ip link set br-ns.10 up
sudo ip link set br-ns.20 up
...
sudo ip -n ns1 route add default via 10.0.10.254
...
sudo ip -n ns2 route add default via 10.0.10.254
...
sudo ip -n ns3 route add default via 10.0.20.254
...
sudo ip -n ns4 route add default via 10.0.20.254
