...
sudo iptables -t filter -D FORWARD -i wg0-test -s 10.0.0.2/32 -j ACCEPT 2>/dev/null || true
sudo iptables -t filter -D FORWARD -o wg0-test -d 10.0.0.2/32 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT 2>/dev/null || true
sudo iptables -t nat -D POSTROUTING -o wlp3s0 -s 10.0.0.2/32 -j MASQUERADE 2>/dev/null || true
...
sudo ip netns exec wg-client wg set wg0 private-key /tmp/private.ns peer $(cat /tmp/public.host) endpoint 192.168.200.1:51820 allowed-ips 0.0.0.0/0

sudo ip -n wg-client route replace default dev wg0

sudo iptables -t filter -P FORWARD DROP
sudo iptables -t filter -A FORWARD -i wg0-test -s 10.0.0.2/32 -j ACCEPT
sudo iptables -t filter -A FORWARD -o wg0-test -d 10.0.0.2/32 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

sudo iptables -t nat -A POSTROUTING -o wlp3s0 -s 10.0.0.2/32 -j MASQUERADE
